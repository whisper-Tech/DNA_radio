...
class RadioState extends EventEmitter {
  ...
  async immediateTransition(removedSongId) {
    ...
    // Small delay for cross-fade visual effect
    await new Promise(resolve => setTimeout(resolve, this.crossFadeDuration));
  ...
  vote(songId, type) {
    ...
    if (type === 'accept') {
      ...
      if (song.health <= -10 && song.status !== 'removed') {
        ...
        // If this is the current song, trigger immediate transition with cross-fade
        if (song.id === this.playlist[this.currentIndex].id) {
          ...
          this.immediateTransition(song.id);
