...
app.post('/api/add-song', (req, res) => {
  ...
  const song = radio.manualAdd(title, artist, youtubeId, duration);
...
io.on('connection', (socket) => {
  ...
  socket.on('vote', ({ songId, type }) => {
    ...
    radio.vote(songId, type, socket.id);
  });

  // Handle AI Suggestion Selection
  socket.on('select_suggestion', ({ index }) => {
    console.log(`[SYNC] Suggestion ${index} selected by ${socket.id}`);
    radio.selectAISuggestion(index, socket.id);
  });
